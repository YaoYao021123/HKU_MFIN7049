...
| 模块 6 | 银行、金融机构与监管 | `mfin7049/Slides - 9.pdf`、`mfin7049/Slides - 10.pdf`、`mfin7049/Slides - 11.pdf`、`mfin7049/Teaching Notes - 9.pdf`、`HW3/Assignment 3.pdf` | 掌握银行作为特殊金融机构、`tradability / redeemability`、`banknote / bond / loan`、bank run 与监管逻辑，以及 `bond vs banknote` 和 redemption threshold 这类 institution-level stress 题型 |
...
| `HW3/Assignment 3.pdf` | Q1 | 模块 6 | 把 `tradability`、`redeemability`、liquidity shock、search friction 和 bargaining power 放进同一个 valuation problem | 作为模块 6 的完整例题，训练 `bond vs banknote` 的 fixed-point valuation |
| `HW3/Assignment 3.pdf` | Q2 | 模块 6、模块 9 | mutual fund redemption、first-mover incentive、run threshold 和 fire-sale dilution | 作为模块 6 的高阶阈值题，并在客观题冲刺里回收 run logic |
...
- `HW3 Q1/Q2` 只在模块 6 的 institution/contract 主线中使用，强调 market liquidity、redeemability、first-mover incentive 与 run 逻辑。
